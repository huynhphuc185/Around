...
class InterestCategoryViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        pageIndex = 1
        self.tblView.register(UINib(nibName: "SubCategoryCell", bundle: nil), forCellReuseIdentifier: "hotCell")
        tblView.addInfiniteScrollingWithHandler {
            DispatchQueue.global(qos: .userInitiated).async {
                sleep(3)
                DispatchQueue.main.async { [unowned self] in
                    self.pageIndex = self.pageIndex! + 1
                    DataConnect.getCategoryContent(token_API, country_code: kCountryCode, phone: userPhone_API, id_category: self.id_category!, page: self.pageIndex! ,tab:"HOT",view: self.view,  onsuccess: { data in
        ...
        DataConnect.getCategoryContent(token_API, country_code: kCountryCode, phone: userPhone_API, id_category: id_category!, page: self.pageIndex!,tab:"HOT",view: self.view,  onsuccess: { data in
            if let result = data as? CategoryContent
            {
                if result.type == "product"
                {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "hotCell") as! SubCategoryCell
